function Microphone(_fft) {
    ...
    this.startMic = function (context) {
    ...
    this.mapRawSound = function (_me, _total, _min, _max) {
        ...
        if (self.spectrum.length > 0) {
            ...
            //console.log(self.spectrum[new_freq]);
            // map the volumes to a useful number
            return this.map(self.spectrum[new_freq], 0, self.peak_volume, min, max);
        } else {
            return 0;
        }

    }
    this.map = function map(value, min1, max1, min2, max2) {
    ...
    this.getVol = function () {
        ...
        self.volume = this.map(self.vol, 0, self.peak_volume, 0, 100);
        return self.volume;
    }
